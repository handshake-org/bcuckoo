...
class Cuckoo {
  ...
  constructor(bits, size, ease = 50, legacy = false) {
    ...
    if (ease < 1 || ease > 100)
      throw new Error('Ease must be 1 to 100.');
    ...
    // Sanity check.
    assert(ease !== 50 || this.easiness === (this.mask + 1));
  ...
  verify(key, nonces) {
    assert(Buffer.isBuffer(key));
    assert(nonces instanceof Uint32Array);
...
class Solution extends Struct {
  ...
  toArrayLike(ArrayLike) {
    assert(typeof ArrayLike === 'function');
  ...
  fromString(str) {
    ...
    assert(str.length <= (MAX_SIZE * 10) + (MAX_SIZE - 1) * 2);
